-- Search Testing Query Suite Status
-- Monitor test query coverage and evaluation progress
-- Usage: sqlite3 -header -column data/videos.db < scripts/queries/search_testing.sql
-- .headers on
-- .mode column
SELECT '=== TEST QUERY SUITE STATUS ===' as section;
SELECT 'Total Test Queries' as metric,
    COUNT(*) as value
FROM test_queries
UNION ALL
SELECT 'Query Types',
...
FROM test_queries
UNION ALL
SELECT 'Evaluated Queries',
    COUNT(DISTINCT query_text)
FROM evaluation_results
UNION ALL
SELECT 'Unevaluated Queries',
    (
...
SELECT COALESCE(query_type, 'unknown') as query_type,
    COUNT(*) as query_count
FROM test_queries
GROUP BY query_type
ORDER BY query_count DESC;
SELECT '=== EVALUATION COVERAGE BY QUERY TYPE ===' as section
SELECT tq.query_type,
    ...
    ROUND(
        COUNT(DISTINCT er.query_text) * 100.0 / NULLIF(COUNT(DISTINCT tq.id), 0),
        1
    ) as coverage_pct,
...
SELECT tq.id,
    ...
    ROUND(AVG(CAST(er.relevance_score AS FLOAT)), 2) as avg_relevance,
    CASE
        WHEN COUNT(DISTINCT er.id) > 0 THEN 'Evaluated'
        ELSE 'Pending'
    END as status
FROM test_queries tq
    LEFT JOIN evaluation_results er ON tq.query_text = er.query_text
...
SELECT tq.query_text,
    ...
    SUM(
        CASE
            WHEN er.best_result_position = 1 THEN 1
            ELSE 0
        END
    ) as top1_hits
FROM test_queries tq
...
SELECT expected_channels,
    COUNT(*) as query_count
FROM test_queries
WHERE expected_channels IS NOT NULL
    AND TRIM(expected_channels) != ''
