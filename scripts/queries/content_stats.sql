...
/* Usage: sqlite3 -header -column data/videos.db < scripts/queries/content_stats.sql */
.headers on
.mode column
...
SELECT 'Overall Stats' as metric,
    ROUND(
        AVG(
            CAST(like_count AS FLOAT) / NULLIF(view_count, 0) * 100
        ),
        3
    ) as like_rate_pct,
    ROUND(
        AVG(
            CAST(comment_count AS FLOAT) / NULLIF(view_count, 0) * 100
        ),
        3
    ) as comment_rate_pct,
    COUNT(*) as video_count
...
WITH date_dist AS (
    SELECT strftime('%Y', datetime(published_at, 'unixepoch')) as year,
